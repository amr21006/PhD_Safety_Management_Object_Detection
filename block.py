...
class CAM(nn.Module):
    def __init__(self, inc, c2, fusion='weight'):
        super().__init__()
        
        assert fusion in ['weight', 'adaptive', 'concat']
        self.fusion = fusion
        
        self.conv1 = DSConv2D(inc, inc, 3, 1, None, 1, d=1)
        self.conv2 = DSConv2D(inc, inc, 3, 1, None, 1, d=3)
        self.conv3 = DSConv2D(inc, inc, 3, 1, None, 1, d=5)
        
        self.fusion_1 = DSConv2D(inc, c2, 3)
        self.fusion_2 = DSConv2D(inc, c2, 3)
        self.fusion_3 = DSConv2D(inc, c2, 3)

        if self.fusion == 'adaptive':
            self.fusion_4 = DSConv2D(inc * 3, 3, 1)
...
class Conv_LR(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.act = Acon_FReLU(c2) if act is True else act if isinstance(act, nn.Module) else nn.Identity()
...
class DSConv(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=None, dilation=1, groups=1, padding_mode='zeros', bias=False, block_size=16, KDSBias=False, CDS=False):
...
class CoT2f(nn.Module):
    ...
    def __init__(self, c1, c2, n, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv_LR(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv_LR((2 + n) * self.c, c2, 1)
        self.m = nn.ModuleList(CoTBottleneck(self.c, self.c, shortcut, e=1) for _ in range(n))
...
class C3SPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
...
class Detect_Bottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, k=(3, 3)):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__()
        self.cv1 = DSConv2D(c1, c2, k[0])
        self.cv2 = DSConv2D(c2, c2, k[1])
        self.add = shortcut and c1 == c2
...
class SPPFCSP(nn.Module):
    ...
    def __init__(self, c1, c2, g=1, k=5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c1 // 2)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = nn.Conv2d(c1, c_, 1, 1, bias=False)
        self.cv3 = nn.Conv2d(4* c_, c_, 1, 1, bias=False)
...
class DSConv2D(Conv):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.conv = DSConv(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d)